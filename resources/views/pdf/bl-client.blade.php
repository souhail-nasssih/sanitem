...
<head>
    ...
    <title>Bon de Livraison Client - {{ $bonLivraison->numero_bl }}</title>
    <style>
        @page {
            margin: 15mm;
            size: A4;
...
<body>
    ...
    <table class="top-table">
        <tr>
            ...
            <td class="top-right">
                <div class="doc-title">BON DE LIVRAISON CLIENT</div>
                <div class="doc-number">N° {{ $bonLivraison->numero_bl }}</div>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="top-left">
                <div class="info-block">
                    <div><strong>Date du Bon de Livraison:</strong> {{ \Carbon\Carbon::parse($bonLivraison->date_bl)->format('d/m/Y') }}</div>
                    <div><strong>Référence du Bon de Livraison:</strong> {{ $bonLivraison->numero_bl }}</div>
                </div>
            </td>

            <!-- BOTTOM RIGHT: Client block (under the title/number) -->
            <td class="top-right">
                <div class="destinataire info-block" style="text-align:left;">
                    <div style="font-weight:bold; margin-bottom:6px;">Client:</div>
                    <div><span class="label">Nom</span> <strong>{{ $bonLivraison->client->nom_complet }}</strong></div>

                    {{-- Preserve address line breaks if present --}}
                    <div><span class="label">Adresse</span>
                        {!! nl2br(e($bonLivraison->client->adresse ?? '')) !!}
                    </div>

                    <div><span class="label">Tel</span> {{ $bonLivraison->client->numero_tel ?? '' }}</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="products">
        ...
        <tbody>
            @php $rows = 0; @endphp
            @foreach ($bonLivraison->detailBLs as $detail)
                <tr>
                    <td class="text-center">{{ $detail->produit->reférence ?? '' }}</td>
                    <td class="text-left">{{ $detail->produit->discription ?? '' }}</td>
    ...
    <!-- TOTAL -->
    @php
        $totalHT = $bonLivraison->detailBLs->sum(function($detail) {
